...
export async function POST(req: Request) {
  ...
  const { data: vtok, error: e1 } = await supabaseAdmin
    ...
    .eq('token', token)
    .single()
  if (e1 || !vtok) return NextResponse.json({ error: 'invalid token' }, { status: 400 })
  if (vtok.used) return NextResponse.json({ error: 'token used' }, { status: 400 })
  if (new Date(vtok.expires_at).getTime() < Date.now()) return NextResponse.json({ error: 'token expired' }, { status: 400 })
  ...
  const url = `${process.env.NEXT_PUBLIC_APP_URL || ''}/api/verify/view?token=${vt}`
  return NextResponse.json({ viewUrl: url, expiresAt: viewExp.toISOString() })
}
